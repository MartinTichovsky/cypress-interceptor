...
describe("Edge Cases and Error Scenarios", () => {
        ...
        it("should handle XHR with progress events", () => {
            cy.window().then((win) => {
                return new Promise((resolve) => {
                    ...
                    xhr.onload = () => {
                        expect(progressEvents).to.be.greaterThan(0);
                        resolve(undefined);
                    };
                });
            });
        ...
        it("should handle multiple concurrent fetch requests", () => {
            cy.window().then(async (win) => {
                ...
                const requests = [
                    ...
                    win.fetch(
                        ...getInitForFetchFromParams(entry2, getParamsFromDynamicRequest(entry2))
                    ),
                    win.fetch(
                        ...getInitForFetchFromParams(entry3, getParamsFromDynamicRequest(entry3))
                    )
                ];
    ...
    describe("Error Handling Scenarios", () => {
        it("should handle network errors in fetch", () => {
            cy.window().then(async (win) => {
                return win
                    .fetch("http://localhost:3001/invalid-url")
                    .then(() => {
        ...
        it("should handle an invalid url for WebSocket", () => {
            cy.window().then((win) => {
                return new Promise((resolve) => {
                    const ws = new win.WebSocket("ws://localhost:3001/ws/");
