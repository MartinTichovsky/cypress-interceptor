...
cy.watchTheConsole().then(watchTheConsole => {
    expect(
        watchTheConsole.records.filter((entry) => entry.type === ConsoleLogType.ConsoleError).length
    ).to.eq(0);
    expect(watchTheConsole.error).to.have.length(0);
    expect(watchTheConsole.jsError).to.have.length(0);
    expect(watchTheConsole.log).to.have.length.greaterThan(0);
});
```
...
Return all logged JavaScript errors.
